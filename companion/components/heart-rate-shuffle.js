...
class HeartRateShuffleAlgorithm {
  ...
  async pollCurrentSongForQueueingNextTrack() {
    ...
    console.log(
      `Ready to queue next song: ${this.app.getInferredTimeLeftOnCurrentTrack() / 1000} seconds left on current track`
    );
  ...
  startPollingCurrentSong() {
    this.pollCurrentSongFinishedIntervalId = setInterval(
      () => this.pollCurrentSongForQueueingNextTrack(),
      POLL_SONG_INTERVAL
    );
  }
  ...
  matchSongBasedOffHeartRate() {
    ...
    console.log(
      `Current heart rate intensity: ${currentHeartRateIntensity.toFixed(
        2
      )} (${currentHeartRate} bpm) (${maxHeartRate} max) (${restingHeartRate} resting)`
    );
    ...
    const songsWithRelativeIntensity = songsNotInHistory.map((i) => {
      return { id: i.id, relativeIntensity: i.energy - currentHeartRateIntensity };
    });

    const closestSongsToIntensity = songsWithRelativeIntensity.sort(
      (a, b) => Math.abs(a.relativeIntensity) - Math.abs(b.relativeIntensity)
    );
